...
// Abort a hung upstream call after this long — per fetch, so a model that
// hangs gets cut off and the chain moves on to the next candidate.
...
async function postChat(apiKey, body) {
  ...
  try {
    ...
    if (err.name === 'TimeoutError' || err.name === 'AbortError') {
      throw new Error(`Banana2556 timed out after ${FETCH_TIMEOUT_MS / 1000}s (model ${body.model})`);
    }
    throw err;
  }
}
