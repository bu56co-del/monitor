...
async function scrapeCreatives(pageId, { browser, max = 200 } = {}) {
  ...
  try {
    ...
    // proxy for loaded-ad count. Up to 30 scroll passes; bail only after
    // 3 consecutive scrolls with no new ads (FB sometimes pauses lazy-load).
    let lastSeen = 0;
    let stagnant = 0;
    for (let i = 0; i < 30; i++) {
