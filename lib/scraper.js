...
async function scrapeCreatives(pageId, { browser, max = 1000 } = {}) {
  ...
  try {
    ...
    // Scroll until FB's lazy load runs out OR we hit the max cap. Default
    // max is 1000 (effectively unlimited for our targets — Perle ~180 is
    // the largest). 50 scroll passes × 4s wait = up to ~3.5 min for
    // hundreds of ads; we bail only after 4 consecutive stagnant scrolls
    // because FB sometimes pauses lazy-load for a beat.
    let lastSeen = 0;
    let stagnant = 0;
    for (let i = 0; i < 50; i++) {
      ...
      if (seen >= max) break;
      stagnant = seen === lastSeen ? stagnant + 1 : 0;
      if (stagnant >= 4) break;
      lastSeen = seen;
