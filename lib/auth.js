...
// that browsers can treat as a host — closes the open-redirect hole.
...
// In-memory per-IP failed-login tracker. Single-instance only (resets on
// restart, not shared across replicas) — adequate for one Render web
// service. After LOGIN_MAX_FAILS failures inside LOGIN_WINDOW_MS, the IP is
// locked out for LOGIN_LOCKOUT_MS. A successful login clears the counter.
...
function recordFailedLogin(ip) {
  ...
  if (loginAttempts.size > 5000) {
    for (const [k, v] of loginAttempts) {
      if ((!v.lockedUntil || now > v.lockedUntil) && now - v.first > LOGIN_WINDOW_MS) {
  ...
  if (!rec || now - rec.first > LOGIN_WINDOW_MS) {
    rec = { fails: 0, first: now, lockedUntil: 0 };
  }
  rec.fails += 1;
