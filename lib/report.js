...
async function generateReport(opts = {}) {
  ...
  // Allow callers (e.g. the dashboard's secondary button) to override the
  // env-var default provider per call. Look up the adapter's char budget
  // first so the prompt is sized appropriately.
  const providerName = opts.provider || ai.provider();
  const { charBudget } = ai.getProviderMeta(providerName);

  const prompt = Number.isFinite(charBudget)
    ? buildPromptWithinBudget(digest, SELF_PAGE_ID, opts.landingDiffs || [], charBudget)
    : buildPrompt(digest, SELF_PAGE_ID, opts.landingDiffs || []);

  const aiOpts = { system: SYSTEM_PROMPT, maxTokens: 4000, temperature: 0.4 };
  const aiOut = opts.provider
    ? await ai.chatForProvider(opts.provider, prompt, aiOpts)
